 ...
 * @var array                                                     $sections
 * @var \ADP\BaseVersion\Includes\External\AdminPage\Tabs\Options $tab_handler
...
<form method="post">
    ...
    <input type="hidden" name="tab" value="<?php echo $tab_handler::get_key(); ?>"/>
            ...
            <table class="section-settings">
				...
				if ( isset( $section['templates'] ) && is_array( $section['templates'] ) ) {
					foreach ( $section['templates'] as $template ) {
						$tab_handler->render_options_template( $template, compact('options', 'product_bulk_table_customizer_url', 'category_bulk_table_customizer_url', 'amount_saved_customer_url') );
					}
				}
				?>
            </table>
